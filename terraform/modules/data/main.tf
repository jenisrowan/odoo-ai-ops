...
resource "aws_db_instance" "postgres" {
  ...
  # Teardown must never need a human. No final snapshot, no leftover automated
  # backups, and deletion protection pinned off so an out-of-band change can't
  # turn a slow delete into a failed one. (A Multi-AZ instance still takes RDS
  # several minutes to drop - there is no force-delete in the RDS API.)
